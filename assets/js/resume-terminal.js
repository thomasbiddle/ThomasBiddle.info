...
function Prompt (username, host) {

    this.set_user = function(val) {
        user = val;
        set_prompt();
    };
    this.set_hostname = function(val) {
        hostname = val;
        set_prompt();
    };
    ...
    this.get_prompt = function() {
        var html_val = '<input type="text" id="terminal-input" autocomplete="off"/>';
        return this.ps1 + html_val;
    };

    function set_prompt() {
        this.ps1 = this.user + '@' + this.hostname + ' $ ';
    }

    this.init = function(user_arg, host_arg) {
        this.set_user(user_arg);
        this.set_hostname(host_arg);
        set_prompt();
    };

    this.init(username, host);
...
function Terminal() {
    ...
    function this.init() {
        var p = new Prompt('recruiter', 'resume');
        state = 'terminal'
        ...
        if (input === "about") {
            ...
            if ((input.substring(3, 7) === "root") || (input.substring(3, 8) === "admin") || (input.substring(3, 16) === "administrator")) {
                ...
                if (input.substring(3, input.length).length !== 0) {
                    this.p.set_user(input.substring(3, input.length));
                } else {
                    echo(this.p.get_hostname + "");
                }
            }
            reset_terminal();
            ...
            if (input.substring(9, input.length).length !== 0) {
                this.p.set_hostname(input.substring(9, input.length));
                echo("Message from root@" + this.p.get_hostname + ": Hey, what's going on here!?");
            } else {
...
function returnKey(evt) {
    ...
    if (evt.keyCode === 13) {
        // if (TERM.get_state() == 'mail_app') {
            // TERM.mail_command(document.getElementById('terminal-input').value);
        // } else {
            var t = new Terminal();
            t.command(document.getElementById('terminal-input').value);
        // }
    }
}
